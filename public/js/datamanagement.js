document.addEventListener('DOMContentLoaded', () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      const local = JSON.parse(localStorage.getItem('sipandu_dm_records') || '[]');
      if (Array.isArray(local) && local.length) {
        const key = (r) => (r.opd || '') + '|' + (r.name || '') + '|' + (r.period || '');
        const seen = new Set(records.map(key));
        local.forEach(r => { if (!seen.has(key(r))) records.push(r); });
      }
    } catch(__){}
    try {
      const el = document.getElementById('dm-approved-json');
      const srv = el ? JSON.parse(el.textContent || '[]') : [];
      if (Array.isArray(srv) && srv.length) {
        const key = (r) => (r.opd || '') + '|' + (r.name || '') + '|' + (r.period || '');
        const seen = new Set(records.map(key));
        srv.forEach(r => { if (!seen.has(key(r))) records.push(r); });
      }
    } catch(___){ }
  };
  ...
  const renderTable = (q = '', s = '', opd = '') => {
    ...
    const rows = slice.map((r, idx) => {
      return `
      <tr class="row-accent ${r.status==='Disetujui' ? 'accent-green' : (r.status==='Menunggu Persetujuan' ? 'accent-amber' : 'accent-red')}" data-id="${r.id}">
        <td>${start + idx}</td>
        <td>${r.opd||'-'}</td>
        <td>${r.name||'-'}</td>
        <td>${r.period||'-'}</td>
        <td><span class="status-badge ${r.status==='Disetujui' ? 'status-complete' : (r.status==='Menunggu Persetujuan' ? 'status-menunggu' : 'status-rejected')}">${r.status||'-'}</span></td>
        <td>${r.pic||'-'}</td>
        <td>${(r.createdAt?new Date(r.createdAt):new Date()).toLocaleDateString('id-ID')}</td>
        <td>
          ...
          <button class="btn btn-outline btn-sm view-btn" data-id="${r.id}"><i class="fas fa-eye"></i></button>
        </td>
      </tr>
    `}).join('');
    ...
    if (table) table.querySelectorAll('.view-btn').forEach(b=>{
      b.onclick = () => {
        ...
        const history = records.filter(x=> x.opd===item.opd && x.name===item.name && x.id!==item.id).sort((a,b)=> new Date(b.createdAt) - new Date(a.createdAt));
        const histHtml = history.length ? history.map(h=> `
          <div class="file-item"><div><div class="thread-title">${h.name}</div><div class="thread-meta">${h.opd} • ${h.period}</div></div><div class="file-meta"><span>${h.status}</span><span>${h.pic||'-'}</span></div></div>
        `).join('') : '<div class="file-item"><div>Tidak ada riwayat perubahan</div></div>';
        body.innerHTML = `
          <div class="detail-grid">
            <div><div class="detail-label">OPD</div><div class="detail-value">${item.opd||'-'}</div></div>
            <div><div class="detail-label">Nama Data</div><div class="detail-value">${item.name||'-'}</div></div>
            <div><div class="detail-label">Periode</div><div class="detail-value">${item.period||'-'}</div></div>
            <div><div class="detail-label">Status</div><div class="detail-value">${item.status||'-'}</div></div>
            <div><div class="detail-label">Pengaju</div><div class="detail-value">${item.pic||'-'}</div></div>
          </div>
    ...
    const pending = records.filter(r=> r.status==='Menunggu Persetujuan').length;
  ...
  if (saveBtn) saveBtn.onclick = async () => {
    ...
    try {
      const payload = { opd: addOPD.value, name: addName.value, period: addPeriod.value, status: addStatus?.value || 'Menunggu Persetujuan', pic: addPic.value || current.name };
  ...
  const renderPendingReview = () => {
    ...
    const pendingAll = records.filter(r=> r.status==='Menunggu Persetujuan');
    const pending = isSuper ? pendingAll : pendingAll.filter(r=> r.opd === userOPD);
    if (!pending.length) { body.innerHTML = '<div class="file-item"><div>Tidak ada pengajuan menunggu persetujuan</div></div>'; return; }
    const items = pending.map(r=> `
      <div class="review-item" data-id="${r.id}">
        <div class="review-main">
          <div class="review-title">${r.name}</div>
          <div class="review-meta">${r.opd} • ${r.period}</div>
          <div class="chip-row"><span class="chip chip-warning">Menunggu Persetujuan</span></div>
        </div>
    ...
    body.querySelectorAll('.review-approve').forEach(b=>{
      b.onclick = () => {
        ...
        records = records.map(x=> x.id==id ? { ...x, status:'Disetujui' } : x); saveRecords(records);
    ...
    body.querySelectorAll('.review-reject').forEach(b=>{
      b.onclick = () => {
        ...
        records = records.map(x=> x.id==id ? { ...x, status:'Ditolak' } : x); saveRecords(records);
