@extends('layouts.app')
@section('title', 'Portal Dinas')
@section('body-class', 'dashboard-page force-light')

@section('content')
    <div class="page active" id="dinas-status-page">
        <div class="page-header"><h1>Portal Dinas</h1><p>Progres dan kelengkapan data per OPD</p></div>
        <div class="kpi-grid">
            ...
            <div class="kpi-card" id="ds-kpi-pending"><div class="kpi-icon"><i class="fas fa-exclamation-circle"></i></div><div class="kpi-content"><div class="kpi-value">0</div><div class="kpi-label">Menunggu persetujuan</div></div></div>
        </div>
        <div class="card">
            <div class="card-header">
                <div class="ds-hero" style="width:100%">
                    ...
                    <div class="controls-row">
                        ...
                        <select id="ds-status" class="form-control"><option value="">Semua Status</option><option value="Disetujui">Disetujui</option><option value="Menunggu Persetujuan">Menunggu Persetujuan</option><option value="Ditolak">Ditolak</option></select>
                    </div>
                </div>
            </div>
            <div class="card-body">
...
#dinas-status-page #ds-table thead th{cursor:pointer}
#dinas-status-page .status-badge{display:inline-flex;align-items:center;padding:4px 8px;border-radius:9999px;font-size:12px;border:1px solid #e5e7eb;background:#fff;color:#334155}
#dinas-status-page .status-badge.status-approved{border-color:#22c55e;color:#166534;background:#dcfce7}
#dinas-status-page .status-badge.status-rejected{border-color:#ef4444;color:#7f1d1d;background:#fee2e2}
#dinas-status-page .status-badge.status-menunggu{border-color:#f59e0b;color:#7c2d12;background:#fffbeb}
...
document.addEventListener('DOMContentLoaded', function(){
  var table=document.getElementById('ds-table'); if(!table) return;
  var search=document.getElementById('ds-search');
  var statusSel=document.getElementById('ds-status');
  var sortKey='created_at', sortDir='desc';
  var kTotal=document.querySelector('#ds-kpi-total .kpi-value');
  var kComplete=document.querySelector('#ds-kpi-complete .kpi-value');
  var kProgress=document.querySelector('#ds-kpi-progress .kpi-value');
  var kPending=document.querySelector('#ds-kpi-pending .kpi-value');
  function badge(s){var cls='status-badge';if(s==='Disetujui')cls+=' status-approved';else if(s==='Ditolak')cls+=' status-rejected';else cls+=' status-menunggu';return '<span class="'+cls+'">'+s+'</span>';}
  function buildHeader(){table.innerHTML='<thead><tr>'+
    '<th data-sort="dinas">Dinas</th>'+
    '<th data-sort="nama_dinas">Nama Dinas</th>'+
    '<th data-sort="nama_data">Nama Data</th>'+
    '<th data-sort="status">Status</th>'+
    '</tr></thead><tbody></tbody>';
    table.querySelectorAll('th[data-sort]').forEach(function(th){ th.addEventListener('click', function(){ var k=th.getAttribute('data-sort'); if(sortKey===k){ sortDir = (sortDir==='asc') ? 'desc' : 'asc'; } else { sortKey=k; sortDir='asc'; } load(); }); });
  }
  function renderRows(rows){ var tbody=table.querySelector('tbody'); if(!tbody) return; tbody.innerHTML=(rows||[]).map(function(r){ return '<tr><td>'+ (r.dinas||'-') +'</td><td>'+ (r.nama_dinas||'-') +'</td><td>'+ (r.nama_data||'-') +'</td><td>'+ badge(r.status||'-') +'</td></tr>'; }).join(''); }
  function updateKpis(rows){ try{ var list=rows||[]; var c=list.filter(function(r){return r.status==='Disetujui';}).length; var p=list.filter(function(r){return (r.status!=='Disetujui'&&r.status!=='Ditolak'&&r.status!=='Menunggu Persetujuan');}).length; var m=list.filter(function(r){return r.status==='Menunggu Persetujuan';}).length; if(kComplete){ kComplete.textContent=String(c); } if(kProgress){ kProgress.textContent=String(p); } if(kPending){ kPending.textContent=String(m); } }catch(e){} }
  async function refreshTotalDinas(){ try{ var r=await fetch('/forum/dinas',{headers:{'Accept':'application/json'}}); var d=await r.json(); if(Array.isArray(d)&&kTotal){ kTotal.textContent=String(d.length); } }catch(e){ if(kTotal){ kTotal.textContent='0'; } } }
  async function load(){ var qs='?q='+encodeURIComponent(search.value||'')+'&status='+(statusSel.value||'')+'&sort='+encodeURIComponent(sortKey)+'&dir='+encodeURIComponent(sortDir); try{ var res=await fetch('/dinas-status/data'+qs,{headers:{'Accept':'application/json'}}); var data=await res.json(); renderRows(data); updateKpis(data); }catch(_){ renderRows([]); updateKpis([]); } refreshTotalDinas(); }
  var timer=null; function startAuto(){ if(timer) clearInterval(timer); timer=setInterval(load, 10000); }
  buildHeader(); load(); startAuto();
  search?.addEventListener('input', function(){ load(); });
  statusSel?.addEventListener('change', function(){ load(); });
