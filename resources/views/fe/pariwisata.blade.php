...
.pw-card .card-header{padding:18px 22px;display:flex;align-items:center;justify-content:space-between;background:linear-gradient(90deg,#0ea5e9,#1d4ed8);color:#ffffff;border-radius:16px 16px 0 0}
...
.pw-card .card-actions .btn{border-radius:9999px;padding:8px 14px;font-weight:600;background:#0ea5e9;color:#fff;border:1px solid #0284c7;box-shadow:0 2px 6px rgba(2,132,199,.2)}
.pw-tabs{display:flex;gap:8px;align-items:center;justify-content:flex-start;margin:14px 0}
.pw-tabs .seg{display:flex;gap:16px;background:#eef4ff;border:1px solid #cfe0ff;border-radius:9999px;padding:12px 16px;width:100%;justify-content:space-between}
.pw-tabs .seg .btn{border-radius:9999px;padding:12px 28px;font-weight:700;transition:all .2s ease;font-size:15px;flex:1 1 auto;text-align:center;min-width:140px}
.pw-tabs .seg .btn-primary{background:#ffffff;color:#1e3a8a;border:1px solid #cfe0ff;box-shadow:0 2px 6px rgba(29,78,216,.12),0 0 0 3px rgba(207,224,255,.6)}
.pw-tabs .seg .btn-outline{background:#eaf2ff;color:#1e3a8a;border:1px solid #cfe0ff}
.pw-tabs .seg .btn:hover{transform:translateY(-0.5px)}
.pw-panel{margin:16px;border-radius:16px;padding:16px;border:1px solid #7dd3fc;background:#e0f2fe}
...
.pw-table{width:100%;border:1px solid #7dd3fc;border-radius:12px;overflow:hidden;border-collapse:separate;border-spacing:0;table-layout:fixed}
.pw-table thead th{background:#cffafe;color:#0f172a;font-weight:600}
.pw-table th,.pw-table td{border-bottom:1px solid #7dd3fc;border-right:1px solid #7dd3fc;padding:10px 12px}
.pw-table thead tr th:first-child{border-left:1px solid #7dd3fc}
.pw-table tbody tr td:first-child{border-left:1px solid #7dd3fc}
    ...
    <div class="page active" id="pariwisata-page">
      <div class="card pw-card">
        <div class="card-header">
          ...
          <div class="card-actions"><button class="btn btn-sm" id="pw-add"><i class="fas fa-plus"></i> Tambah Data</button></div>
        </div>
        <div class="card-body">
          <div class="pw-tabs"><div class="seg"><button class="btn btn-primary btn-sm" id="pw-tab-ako">Akomodasi</button><button class="btn btn-outline btn-sm" id="pw-tab-wis">Wisatawan</button><button class="btn btn-outline btn-sm" id="pw-tab-jen">Objek (Jenis)</button><button class="btn btn-outline btn-sm" id="pw-tab-kec">Objek (Kecamatan)</button><button class="btn btn-outline btn-sm" id="pw-tab-pem">Pemandu</button></div></div>
          ...
          <div id="pw-wis" style="display:none;">
            ...
            <div class="table-wrap"><table class="table table-compact pw-table"><thead><tr><th>Tahun</th><th colspan="2">Wisatawan</th><th>Jumlah</th></tr><tr><th></th><th>Mancanegara</th><th>Domestic</th><th></th></tr></thead><tbody id="pw-wis-tbody"></tbody></table></div>
          </div>
          ...
          <div id="pw-jen" style="display:none;">
            ...
            <div class="table-wrap"><table class="table table-compact pw-table"><thead><tr><th>Jenis Objek Wisata</th><th colspan="2">Status Objek Wisata</th><th>Jumlah</th></tr><tr><th></th><th>Dikomersilkan</th><th>Belum Dikomersilkan</th><th></th></tr></thead><tbody id="pw-jen-tbody"></tbody></table></div>
          </div>
          ...
          <div id="pw-pem" style="display:none;">
            ...
            <div class="table-wrap"><table class="table table-compact pw-table"><thead><tr><th>No</th><th>Uraian</th><th>2019</th><th>2020</th><th>2021</th><th>2022</th><th>2023</th></tr></thead><tbody id="pw-pem-tbody"></tbody></table></div>
          </div>
...
function renderAko(){var tb=document.getElementById('pw-ako-tbody');if(!tb)return;tb.innerHTML=akoRows.map(function(r){return '<tr><td>'+r.no+'</td><td>'+r.kecamatan+'</td><td>'+r.h22+'</td><td>'+r.k22+'</td><td>'+r.t22+'</td><td>'+r.h23+'</td><td>'+r.k23+'</td><td>'+r.t23+'</td></tr>';}).join('');}
function renderWis(){var tb=document.getElementById('pw-wis-tbody');if(!tb)return;tb.innerHTML=wisRows.map(function(r){return '<tr><td>'+r.tahun+'</td><td>'+r.manca+'</td><td>'+r.dom+'</td><td>'+r.jumlah+'</td></tr>';}).join('');}
function renderJen(){var tb=document.getElementById('pw-jen-tbody');if(!tb)return;tb.innerHTML=jenRows.map(function(r){return '<tr><td>'+r.jenis+'</td><td>'+r.kom+'</td><td>'+r.bel+'</td><td>'+r.jumlah+'</td></tr>';}).join('');}
function renderKec(){var tb=document.getElementById('pw-kec-tbody');if(!tb)return;tb.innerHTML=kecRows.map(function(r){return '<tr><td>'+r.no+'</td><td>'+r.kecamatan+'</td><td>'+r.kom22+'</td><td>'+r.kom23+'</td><td>'+r.bel22+'</td><td>'+r.bel23+'</td><td>'+r.jumlah+'</td></tr>';}).join('');}
function renderPem(){var tb=document.getElementById('pw-pem-tbody');if(!tb)return;tb.innerHTML=pemRows.map(function(r){return '<tr><td>'+r.no+'</td><td>'+r.uraian+'</td><td>'+r.y2019+'</td><td>'+r.y2020+'</td><td>'+r.y2021+'</td><td>'+r.y2022+'</td><td>'+r.y2023+'</td></tr>';}).join('');}
function init(){renderAko();renderWis();renderJen();renderKec();renderPem();}
init();

function toggleTab(active){['pw-ako','pw-wis','pw-jen','pw-kec','pw-pem'].forEach(function(id){document.getElementById(id).style.display=id===active?'block':'none';});var ids=['pw-tab-ako','pw-tab-wis','pw-tab-jen','pw-tab-kec','pw-tab-pem'];ids.forEach(function(i){var b=document.getElementById(i);b.classList.add('btn-outline');b.classList.remove('btn-primary');});var map={'pw-ako':'pw-tab-ako','pw-wis':'pw-tab-wis','pw-jen':'pw-tab-jen','pw-kec':'pw-tab-kec','pw-pem':'pw-tab-pem'};var act=document.getElementById(map[active]);act.classList.add('btn-primary');act.classList.remove('btn-outline');}
...
document.getElementById('pw-add')?.addEventListener('click',function(){var current=document.querySelector('.pw-card .pw-panel:not([style*="display:none"])');if(current){current.style.display='none';}
...
document.getElementById('ako-cancel')?.addEventListener('click',function(){document.getElementById('pw-ako-panel').style.display='none';});
document.getElementById('wis-cancel')?.addEventListener('click',function(){document.getElementById('pw-wis-panel').style.display='none';});
document.getElementById('jen-cancel')?.addEventListener('click',function(){document.getElementById('pw-jen-panel').style.display='none';});
document.getElementById('kec-cancel')?.addEventListener('click',function(){document.getElementById('pw-kec-panel').style.display='none';});
document.getElementById('pem-cancel')?.addEventListener('click',function(){document.getElementById('pw-pem-panel').style.display='none';});
